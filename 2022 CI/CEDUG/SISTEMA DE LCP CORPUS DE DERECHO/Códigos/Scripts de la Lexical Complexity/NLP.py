...
#BLOQUE 3
#indir = 'textos_analizar/Ingeniería en Sistemas/5'
                ...
                for index, f in enumerate(filenames):
                    ...
                    if (index == 0 and cont == 0):
                        df = pd.DataFrame({name: [autoreadability[0], ssreadability[0], lexcomplexity[2], lexcomplexity[1],
                                                  ...
                                                  pmarks, mltd, c, s, m, contCconj, contSconj, contPropn, contPron, contNoun,
                                                  contVerb, contAdp, contAux, contDet, contAdv, contAdj]},
                                          index='N_charac N_w N_dcw N_cw N_lfw N_rw N_s N_cs LDI ILFW LC SSR ASL CS SCI ARI PM MTLD CARRERA '
                                                'SEMESTRE MATERIA CCONJ SCONJ PROPN PRON NOUN VERB ADP AUX DET ADV ADJ'.split())
                        cont+=1
                    else:
                        df[name] = ([autoreadability[0], ssreadability[0], lexcomplexity[2], lexcomplexity[1], lexcomplexity[0],
                                     ...
                                     autoreadability[1], pmarks, mltd, c, s, m, contCconj, contSconj, contPropn, contPron, contNoun,
                                                  contVerb, contAdp, contAux, contDet, contAdv, contAdj])
                    print(df)
                    file.close()
            indir = raiz + '/' + c
...
print("\n La tabla final es:\n ")
print(new_df)
new_df.to_excel('resultados/Metricas_Stanford.xlsx', sheet_name='Hoja1')

datos = pd.read_excel('resultados/Metricas_Stanford.xlsx',engine="openpyxl")
rs = pd.DataFrame(datos)

#TABLA COMPACTA POR MATERIAS
print("Tabla compacta por materias")
rsMateria = rs.groupby(['MATERIA'])[['N_charac', 'N_w', 'N_dcw', 'N_cw', 'N_lfw', 'N_rw', 'N_s', 'N_cs', 'LDI',
                                     'ILFW', 'LC', 'SSR', 'ASL', 'CS', 'SCI', 'ARI', 'PM', 'MTLD', 'CCONJ', 'SCONJ',
                                     'PROPN', 'PRON', 'NOUN', 'VERB', 'ADP', 'AUX', 'DET', 'ADV', 'ADJ']].sum()
dfMateria = pd.DataFrame(rsMateria)
print(dfMateria)
...
dfMateria.to_excel(writer,'Tabla compacta materias')
