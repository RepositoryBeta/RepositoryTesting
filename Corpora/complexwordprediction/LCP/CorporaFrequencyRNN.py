...
class CorporaFrequencyRNN(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.chk_bert = tk.Checkbutton(self, text="Bert",variable=self.opcionBert, onvalue=1, offvalue=0)
        self.chk_roberta = tk.Checkbutton(self, text="Roberta",variable=self.opcionRoberta, onvalue=1, offvalue=0)
        self.chk_avgbert = tk.Checkbutton(self, text="Average Bert",variable=self.opcionAvgBert, onvalue=1, offvalue=0)
        self.chk_avgroberta = tk.Checkbutton(self, text="Average Roberta",variable=self.opcionAvgRoberta, onvalue=1, offvalue=0)
        #Bert Spanish
        ...
        self.model_en = BertModel.from_pretrained("bert-base-uncased")
        self.lenSentence = []
    ...
    def init(self):
        ...
        label_title = tk.Label(self, text='Feature Of The Words With Bert or XLM RoBERTa', font="Helvetica 12 bold")
                        ...
                        if self.var.get() == 1:
                            ...
                            #avgTokenBert,avgBert
                                if self.opcionAvgBert.get()==1:
                                    ...
                                    print("Type Data CLS: >> "+str(type(df['bert_cls'])))
                                ...
                                if self.opcionAvgBert.get()==1 and self.opcionAvgRoberta.get()==1:
                                    df = pd.DataFrame(
                                        ...
                                        columns=['id','token', 'abs_frecuency',
                                                ...
                                                'number_synonyms','number_hyponyms','number_hypernyms', 'Part_of_speech',
                                                'freq_relative_word_before','freq_relative_word_after','len_word_before',
                                                'len_word_after','mtld_diversity','propn','aux','verb','adp','noun','nn','sym','num','complexity','bert_cls','bert_token','roberta_s','roberta_token']
                                    )
