...
from conver import prepare_data
import os
from gpt import evaluar
...
PROMPTS_BASE = {
    ...
    "PRM_ZS_TASK2_1_V1": "To retrieve up to 5 difficult terms in a given passage from a scientific abstract. Rank the list of difficult terms with corresponding scores on the scale 1-3 where 3 is the most difficult term.  you have a maximum of 250 tokens you can t go over that.\n##\n",
...
def generate_prompts(row, prompt_selected = ""):
    ...
    prompt = "{}Context: {}\nText: {}\n\n###\n\n".format(header, row["query_text"], row["source_snt"])
    return prompt
...
def process_data(data):
    ...
    for index, row in data.iterrows():
        ...
        try:    
            prompt_selected = "PRM_FS_TASK2_2_V1"
            params = {"table": "result_task2_2", "where": {"snt_id": row["snt_id"], "promptID": prompt_selected}}
            ...
            if True:
                ...
                to_save_task2_1, to_save_task2_2 = format_response(resultados, {"snt_id": snt_id}, total_tokens, prompt_selected, task2_2_cond=True)

                insert_record_result(new_exec)
                # foreach(insert_record_result, to_save_task2_1)
                foreach(insert_record_result, to_save_task2_2)

                print(Fore.GREEN+"Record {} processed and saved successfully".format(row["snt_id"])+Fore.RESET)
                # time.sleep(5)

                # sys.exit()
            # else:
            #     print("{}: has already been processed".format(row["snt_id"]))

        except openai.error.RateLimitError as limit_rate_error:
            print(Fore.RED+"Error during processing of {}: {}".format(row["snt_id"],"Error openai -> " + str(limit_rate_error))+Fore.RESET)
            sys.exit()
        except openai.error.OpenAIError as error_openai:
            print(Fore.RED+"Error during processing of {}: {}".format(row["snt_id"],"Error openai -> " + str(error_openai))+Fore.RESET)
            sys.exit()
        except Exception as e:
            new_exec.update({"state": "error"})
            insert_record_result(new_exec)
            print(Fore.RED+"Error during processing of {}: {}".format(row["snt_id"], e)+Fore.RESET)
            print(index)
            # if count != 1: sys.exit()
        count += 1
        # if count == 4: sys.exit()
